...
import {
    ...
    useGetByIdProductImage,
    useRemoveProductImage,
    useUpdateProductImage,
} from '@/loaders/product-image.loader';
...
const ProductGallery = ({ choosedColor }: Props) => {
    ...
    const currentProductImage = useGetByIdProductImage({
        id: idImage,
        config: {
            onSuccess: (response) => {
                setFileList([
        ...
        config: {
            onSuccess: (_) => {
                ...
                notification.success({
                    message: t('product.create_image_success'),
                });
    ...
    const updateProductImage = useUpdateProductImage({
        id: currentProductImage?.data?._id,
        config: {
            onSuccess: (_) => {
                queryClient.invalidateQueries([CACHE_COLOR.GET_BY_ID]);

                notification.success({
                    message: t('product.update_image_success'),
                });
    ...
    const deleteProductImage = useRemoveProductImage({
        config: {
            onSuccess: () => {
                queryClient.invalidateQueries([CACHE_COLOR.GET_BY_ID]);

                notification.success({
                    message: t('product.delete_image_success'),
                });
            },
            onError: (error: any) => {
                notification.error({
            ...
            setFileList([
                {
                    ...
                    name: DEFAULT_NAME_FILE_LIST,
                    status: DEFAULT_STATUS_FILE_LIST,
                    url: currentProductImage?.data?.picture || noImage,
                },
            ]);
            form.setFieldValue('picture', currentProductImage?.data?.picture);
        }
    ...
    return (
        <>
            <ModalRender
                customHeader={true}
                title={
                    <Typography.Text>{t('product.gallery')}</Typography.Text>
                }
                buttonRender={
                    <Tooltip title={t('product.gallery')}>
                        <Button
                            shape="circle"
                            icon={<FaRegImages />}
                            onClick={handleOpen}
                            ...
                            <Col span={24} md={24} lg={24}>
                                ...
                                >
                                    <Space>
                                        <Tooltip
                                            title={
                                                !currentProductImage?.data?._id
                                                    ? t('product.create_image')
                                                    : t('product.update_image')
                                            }
                                        >
                                            <Button
                                                loading={
                                                    loadingAvatar ||
                                                    createProductImage?.isLoading ||
                                                    updateProductImage?.isLoading
                                                }
                                                type="primary"
                                                shape="circle"
                                                icon={
                                                    currentProductImage?.data
                                                        ?._id ? (
                                                        <EditOutlined />
                                                    ) : (
                                                        <PlusOutlined />
