...
import {
    ...
    useGetByIdLookbookImage,
    useRemoveLookbookImage,
    useUpdateLookbookImage,
} from '@/loaders/lookbook-image.loader';
...
const LookbookGallery = ({ id }: Props) => {
    ...
    const currentLookbookImage = useGetByIdLookbookImage({
        id: idImage,
        config: {
            onSuccess: (response) => {
                setFileList([
    ...
    const createLookbookImage = useCreateLookbookImage({
        config: {
            onSuccess: (_) => {
                queryClient.invalidateQueries([CACHE_LOOKBOOK.GET_BY_ID]);

                notification.success({
                    message: t('lookbook.create_image_success'),
                });
    ...
    const updateLookbookImage = useUpdateLookbookImage({
        id: currentLookbookImage?.data?._id,
        config: {
            onSuccess: (_) => {
                queryClient.invalidateQueries([CACHE_LOOKBOOK.GET_BY_ID]);

                notification.success({
                    message: t('lookbook.update_image_success'),
                });
    ...
    const deleteLookbookImage = useRemoveLookbookImage({
        config: {
            onSuccess: () => {
                queryClient.invalidateQueries([CACHE_LOOKBOOK.GET_BY_ID]);

                notification.success({
                    message: t('lookbook.delete_image_success'),
                });
            },
            onError: (error: any) => {
                notification.error({
            ...
            setFileList([
                {
                    ...
                    name: DEFAULT_NAME_FILE_LIST,
                    status: DEFAULT_STATUS_FILE_LIST,
                    url: currentLookbookImage?.data?.picture || noImage,
                },
            ]);
            form.setFieldValue('picture', currentLookbookImage?.data?.picture);
        }
    };
    ...
    const handleSave = () => {
        form.validateFields()
            .then(async (values) => {
                ...
                if (!currentLookbookImage?.data) {
                    createLookbookImage.mutate({
                        lookbook: currentLookbook?.data?._id,
                        picture: values.picture,
                    });
                } else {
                    updateLookbookImage.mutate({
                        ...values, 
                        lookbook: currentLookbook?.data?._id,
                        picture: values.picture,
                    });
    ...
    return (
        <>
            <ModalRender
                customHeader={true}
                title={
                    <Typography.Text>{t('lookbook.gallery')}</Typography.Text>
                }
                buttonRender={
                    <Tooltip title={t('lookbook.gallery')}>
                        <Button
                            shape="circle"
                            icon={<FaRegImage />}
                            onClick={handleOpen}
                    ...
                    <Form form={form}>
                        <Row gutter={[24, 24]}>
                            ...
                            <Col span={24} md={24} lg={24}>
                                ...
                                >
                                    <Space>
                                        <Tooltip
                                            title={
                                                !currentLookbookImage?.data
                                                    ?._id
                                                    ? t(
                                                          'lookbook.create_image',
                                                      )
                                                    : t(
                                                          'lookbook.update_image',
                                                      )
                                            }
                                        >
                                            <Button
                                                loading={
                                                    loadingAvatar ||
                                                    createLookbookImage?.isLoading ||
                                                    updateLookbookImage?.isLoading
                                                }
                                                type="primary"
                                                shape="circle"
                                                icon={
                            ...
                            <Col span={24} md={24} lg={24}>
                                ...
                                >
                                    {currentLookbook?.data?.lookbookImages.map(
                                        (record: any, index: number) => (
                                            ...
                                            >
                                                ...
                                                <ConfirmRender
                                                    ...
                                                    content={t(
                                                        'lookbook.confirm_delete_image',
                                                    )}
                                                    title={t(
                                                        'lookbook.confirm_delete_title',
                                                    )}
                                                />
                                            </SwiperSlide>
                                        ),
