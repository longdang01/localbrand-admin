...
import { ProductImageProps } from "@/models/product-image";
import { AxiosRequestConfig } from "axios";

const prefix = "colorImages";
...
export const create = async (request: ProductImageProps) => {
    const response = await apiClient?.post(`${prefix}`, request);
    return response.data;
};

export const update = async (id: string, request: ProductImageProps) => {
